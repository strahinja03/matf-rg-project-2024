...
namespace engine::test::app {
    engine::util::Timer MainController::inner_event_timer(0.0f);
    ...
    void MainController::poll_events() {
        ...
        if (platform->key(engine::platform::KEY_F3).state() == engine::platform::Key::State::JustPressed) {
            set_timer(5.0f);
            MainController::m_event1_in_waiting = true;
        }
    ...
    void MainController::set_timer(float delay_seconds) {
        MainController::inner_event_timer = engine::util::Timer(delay_seconds);
    }

    void MainController::scene_event1() { MainController::m_draw_skybox = !MainController::m_draw_skybox; }
